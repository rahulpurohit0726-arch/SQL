...
select e.enrollment_id, e.course_name, s.student_id from enrollments as e;
-- inner join
select s.student_id, s.student_name,e.enrollment_id, e.course_name,
e.student_id from students as s left join  enrollments as e on e.student_id=s.student_id;

select s.student_id, s.student_name,e.enrollment_id, e.course_name,
e.student_id from students as s left join  enrollments as e on s.student_id=e.student_id where enrollment_id is NULL;

SELECT student_name,count(course_name)from students as s join enrollments as e group by s.student_name;

-- offest -> koi value ko skip krena ho to use me lange 
-- limit-> 
select char_length(s.student_name),count(enrollments)from student as s join enrollments as e group by char_length(s.student_name);
-- 183 184 197 511 180

select s.student_id, s.student_name,sum(e.credits),count(enrollment_id)
from students as s left join  enrollments as e on e.student_id=s.student_id group by student_id
