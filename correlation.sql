...
select * from city;
select*from country;
-- corelated sub - is a querry that use 
-- A correlated subquery ->is a subquery (inner query) that uses values from the outer query and is executed 
-- once for each row processed by the outer query.
select avg(salary),name from employee join department ;
